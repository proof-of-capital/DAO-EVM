...
/// @title SetMarketMakerScript
/// @notice Script for setting market maker in DAO
contract SetMarketMakerScript is Script {
    ...
    function run() external {
        ...
        address returnWallet = vm.envAddress("RETURN_WALLET");

        vm.startBroadcast();

        DAO(payable(dao)).setMarketMaker(returnWallet);

        console.log("Market maker set to:", returnWallet);

        vm.sleep(5000);
        vm.stopBroadcast();

        writeDeploymentAddresses(dao, returnWallet);
        console.log("Deployment addresses successfully saved");
    }

    function writeDeploymentAddresses(address dao, address returnWallet) internal {
        ...
        addressesJson = string(abi.encodePacked(addressesJson, '"marketMaker":"', vm.toString(returnWallet), '"}'));
        ...
        string memory envContent =
            string(abi.encodePacked(existingContent, "MARKET_MAKER=", vm.toString(returnWallet), "\n"));
